	...
	def subsync_callback(self, data_mot, data_percep):
		...
		#def scene_callback(self,data):
			...
			while (len(self.time_horizon_motion) < self.time_horizon):
				...
				print("motion ahead len=", len(self.time_horizon_motion))
			...
			for t in range(self.time_horizon):
				for i in range(self.humans -1):
					for j in range(i+1, self.humans):
						for s in range(self.dist_samples):
							...
							ped_k_x_pred = self.time_horizon_motion[t].predictions[i].prediction[s].sample.pose.position.x
							ped_k_y_pred = self.time_horizon_motion[t].predictions[i].prediction[s].sample.pose.position.y
							ped_l_x_pred = self.time_horizon_motion[t].predictions[j].prediction[s].sample.pose.position.x
							ped_l_y_pred = self.time_horizon_motion[t].predictions[j].prediction[s].sample.pose.position.y
							...
							label_qtc_pred.label  = "label: ped1="+str(i)+"/ped2="+str(j)+"/step="+str(t+1) 
							rospy.loginfo("label: ped1="+str(i)+"/ped2="+str(j)+"/step="+str(t+1))
				...
				self.current_motion[0].humans[i].centroid.pose.position.x = self.time_horizon_motion[t].predictions[i].prediction[s].sample.pose.position.x
				self.current_motion[0].humans[i].centroid.pose.position.y = self.time_horizon_motion[t].predictions[i].prediction[s].sample.pose.position.y

				self.current_motion[0].humans[j].centroid.pose.position.x = self.time_horizon_motion[t].predictions[j].prediction[s].sample.pose.position.x
				self.current_motion[0].humans[j].centroid.pose.position.y = self.time_horizon_motion[t].predictions[j].prediction[s].sample.pose.position.y
